...
@TRANSFORMS.register_module()
class BEVLoadMonoViewImageFromFile(LoadMultiViewImageFromFiles):
    """Load multi channel images from a list of separate channel files.

    ``BEVLoadSingleImageFromFiles`` has the following keys for the
    convenience of view transforms in the forward:
        - 'cam2lidar'
        - 'lidar2img'
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        Args:
            results (dict): Result dict containing single-view image filename.
        ...
        filename = [cam_item['img_path']]
        cam2img_array = np.eye(4).astype(np.float32)
        cam2img_array[:3, :3] = np.array(cam_item['cam2img'])[:3, :3].astype(
                np.float32)
        ...
        results['img_path'] = filename
        results['cam2img'] = np.stack([cam2img_array], axis=0)
        results['lidar2cam'] = np.stack([cam_item['lidar2cam']], axis=0)
        results['cam2lidar'] = np.stack([camera2lidar], axis=0)
        results['lidar2img'] = np.stack([cam2img_array @ lidar2cam_array], axis=0)
