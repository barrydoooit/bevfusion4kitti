...
class BEVFusionRandomFlip3D:
    ...
    def __init__(self, flip_vertical=True, flip_horizontal=True):
        self.do_flip_vertical = flip_vertical
        self.do_flip_horizontal = flip_horizontal
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        rotation = np.eye(3)
        if flip_horizontal and self.do_flip_horizontal:
        ...
        if flip_vertical and self.do_flip_vertical:
            rotation = np.array([[-1, 0, 0], [0, 1, 0], [0, 0, 1]]) @ rotation
            if 'points' in data:
