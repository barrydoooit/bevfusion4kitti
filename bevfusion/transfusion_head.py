...
class TransFusionHead(nn.Module):
    ...
    def forward_single(self, inputs, metas):
        ...
        fusion_feat_flatten = fusion_feat.view(batch_size,
                                               fusion_feat.shape[1],
                                               -1)  # [BS, C, H*W]
        bev_pos = self.bev_pos.repeat(batch_size, 1, 1).to(fusion_feat.device) # duplicate for each batch
        ...
        if self.test_cfg['dataset'] == 'nuScenes':
            ...
            or self.test_cfg['dataset'].lower() == 'kitti' \
                or self.test_cfg['dataset'].lower() == 'stf':  # for Pedestrian & Cyclist in Waymo and KITTI, and Pedestrian & RidableVehicle in STF
        ...
        heatmap = heatmap.view(batch_size, heatmap.shape[1], -1) # shape [bs, num_classes, W*H]

        # top num_proposals among all classes
        top_proposals = heatmap.view(batch_size, -1).argsort(
            dim=-1, descending=True)[..., :self.num_proposals] # shape [bs, num_proposals]:  num_proposal (200) points in flattened heatmap
        # top_proposals_class = top_proposals // heatmap.shape[-1] # shape [bs, num_proposals]
        top_proposals_class = torch.div(top_proposals, heatmap.shape[-1], rounding_mode='floor')
        top_proposals_index = top_proposals % heatmap.shape[-1] # shape [bs, num_proposals]
        query_feat = fusion_feat_flatten.gather(
            index=top_proposals_index[:, None, :].expand( # extra dimension in middle
                -1, fusion_feat_flatten.shape[1], -1), # expand dimension to feature channel
            dim=-1, # gather the feature at the 'num_proposal' points of all channels
        )
        self.query_labels = top_proposals_class

        # add category embedding
        one_hot = F.one_hot( #one hot for all proposal points' class
            top_proposals_class,
    ...
    def predict_by_feat(self,
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            if self.test_cfg['dataset'] == 'nuScenes':
                ...
                self.tasks = [ # not used, as nms_type is None
                    dict(
                        num_class=1,
                        class_names=['Car'],
                        indices=[0],
    ...
    def get_targets_single(self, gt_instances_3d, preds_dict, batch_idx):
        ...
        feature_map_size = torch.div(grid_size[:2], 
                                     self.train_cfg['out_size_factor'], rounding_mode='floor')
        ...
        for idx_layer in range(
            ...
            if 'vel' in preds_dict.keys():
                layer_vel = preds_dict['vel'][..., idx_layer *
                                              ...
                                              self.num_proposals, ]
                preds = torch.cat([
                    layer_center, layer_height, layer_dim, layer_rot, layer_vel # code size: 2, 1, 3, 2, 2
                ],
                                  dim=1).permute(
                                      0, 2,
